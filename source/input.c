  ...
  if (N_ncdm > 0){
    ...
    for (n=0; n<N_ncdm; n++){
      class_test(pba->m_ncdm_in_eV[n]<0,
                 errmsg,
                 "You entered a negative non-CDM mass m_ncdm[%d], which makes no sense. This error was not caught in previous CLASS versions because the mass is always squared in the code, so CLASS returned the exact same result from +m_ncdm and -m_ncdm. If you want to define an 'effective negative neutrino mass' in the sense of e.g. 2405.00836 or 2407.10965, you can implement it in a python script following e.g. eq.(3) of 2407.10965",n);
    }
